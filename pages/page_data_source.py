"""
Source de donnees et hypotheses du modele.
"""
...
def render():
    ...
    tab_sim, tab_hyp = st.tabs(["Simulees", "Hypotheses"])
    ...
    # ===================== Onglet Simulees =====================
    with tab_sim:
        st.markdown("### Generation de donnees simulees")
    ...
    with tab_hyp:
        ...
        st.markdown("### Matrice de covariance")

        cov = get_covariance_matrix()
        cov_df = pd.DataFrame(cov * 10000, index=short_names, columns=short_names)

        st.caption("Valeurs en points de base au carre (bps²)")
        st.dataframe(
            cov_df.style.format("{:.0f}").background_gradient(cmap="YlOrRd"),
            use_container_width=True,
        )
