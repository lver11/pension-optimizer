"""
Simulation Monte Carlo pour les projections de portefeuille a long horizon.
"""
...
class MonteCarloResult:
    ...
    def compute_statistics(self) -> Dict[str, float]:
        ...
        cumulative_returns = terminal_assets / self.asset_paths[:, 0] if self.asset_paths[:, 0].mean() > 0 else terminal_assets

        return {
            "median_assets": float(np.median(terminal_assets)),
            "mean_assets": float(np.mean(terminal_assets)),
            "p5_assets": float(np.percentile(terminal_assets, 5)),
            "p95_assets": float(np.percentile(terminal_assets, 95)),
            "median_annual_return": float(np.median(self.return_paths.mean(axis=1))),
            "prob_loss": float(np.mean(terminal_assets < self.asset_paths[:, 0])),
            "prob_ruin": float(np.mean(terminal_assets <= 0)),
        }


class MonteCarloSimulator:
    """Simulateur Monte Carlo pour les projections de portefeuille a long terme."""
    ...
    def __init__(
        ...
        initial_liabilities: float = 0.0,
        liability_growth_rate: float = 0.0,
    ):
    ...
    def simulate(self, horizon_years: int = 20) -> MonteCarloResult:
        ...
        for t in range(horizon_years):
            prev_assets = self.initial_assets if t == 0 else asset_paths[:, t - 1]
            ...
            asset_paths[:, t] = np.maximum(asset_paths[:, t], 0)
